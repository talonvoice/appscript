...
def _initcaches():
    # hack to make startup faster, removes support for third-party ScriptingAdditions for now
    path = '/System/Library/ScriptingAdditions'
    additions = [name.rsplit('.', 1)[0] for name in os.listdir(path) if name.endswith('.osax')]
    _osaxnames.extend(additions)
    _osaxcache.update({name.lower(): [f'{path}/{name}.osax', None] for name in additions})
    _osaxnames.sort()
    return
